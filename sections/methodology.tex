...
\begin{figure*}
\centering
\includegraphics[width=0.95\textwidth]{hist_cl.pdf}
...
\begin{equation}
\chi^{2} = \delta_{X}^{T} C^{-1} \delta_{X},
\end{equation}
where the covariance matrix $C = < \delta_{X} \delta_{X}>$ is constructed from mocks without systematic effects. Fig. \ref{fig:clxmock} shows the cross power spectrum and mean density contrast for the DR9 LRG sample. The shades represent the $1\sigma$ level fluctuations observed in 1000 clean mocks with $\fnl=0$ and $100$.
