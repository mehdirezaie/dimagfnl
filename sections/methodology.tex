...
\end{equation}
\mr{where $\Omega_{m}$ is the matter density, $H_{0}$ is the Hubble constant\footnote{$H_{0}=100~({\rm km}~{\rm s}^{-1})/(h^{-1}{\rm Mpc})$ and $k$ is in unit of $h {\rm Mpc}^{-1}$}, $c$ is the speed of light, and $s$ represents the slope of the number count function, a metric quantifying the response of the number density of galaxies to achromatic changes in brightness \citep{2008PhRvD..77b3512L}. The estimation of $s$ involves shifting all magnitudes by an infinitesimal amount and re-running the color-magnitude selection. It is worth noting that, for a fiber flux-selected sample like the DESI LRG targets, the impact of magnification on fiber flux depends on the shape parameters for each morphology type \citep{zhou2023desi}. In our analysis, we find a slight variation in the parameter $s$ across different imaging regions. Specifically, we observe $s=0.951\pm 0.011$ for BASS+MzLS, $s=0.943 \pm 0.007$ for DECaLS North+DECaLS South, and $s=0.945\pm 0.006$ for DESI\footnote{Private communication with Dr. Rongpu Zhou.}. For consistency, we fix $s$ to the central values in our analysis.} The PNG-induced scale-dependent shift is given by \citep{slosar2008constraints}
\begin{equation}
...
\end{align}
\mr{Figure \ref{fig:mask2pf} illustrates ${\hat \omega}^{\rm window}$ for various masks representing the DESI footprint and its imaging sub-regions. We validate our method by comparing it to an alternative approach that directly employs the mode-mode coupling matrix, as detailed in Appendix \ref{ssec:windowconv}.} 
...
\begin{figure}
    ...
    \caption{\mr{The survey mask correlation functions across the imaging regions comprising the DESI footprint, plotted against angular separation. The inset focuses on correlations within the angular range of $100$ to $180$ degrees.}}
    \label{fig:mask2pf}
\end{figure}
...
\mr{As summarized in Table \ref{tab:contmocksmcmc}, we observe notable shifts in the best-fitting estimates of $\fnl$ resulting from fits to the mean power spectrum of the mocks. Specifically, for the $\fnl=0$ mocks, we obtain $\Delta \fnl=-12$ for non-linear three maps, $-20$ for non-linear four maps, and $-27$ for non-linear nine maps. Larger shifts are evident for $\fnl=76.9$: $\Delta \fnl=-23$ for non-linear three maps, $-39$ for non-linear four maps, and $-72$ for non-linear nine maps.}
