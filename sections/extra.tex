...
The survey window convolution is revisited using the direct mixing matrix. We benchmark our codes against the publicly available software, \textsc{NaMaster}\footnote{\href{https://github.com/LSSTDESC/NaMaster}{https://github.com/LSSTDESC/NaMaster}} \citep[see, also,][]{2019MNRAS.484.4127A}. Figure \ref{fig:window_conv} illustrates the different approaches to account for the survey window for two arbitrary values of $\fnl$. The red shade represents the 68\% dispersion of the $\fnl=76.9$ mocks.}


\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{figures/cl_conv.pdf}
    \caption{\mr{The model power spectrum before and after the survey geometry convolution for $\fnl=0$ and $76.9$. The bottom panel shows the residual error with respect to the \textsc{NaMaster} code.}}
