...
\mr{Our analysis is simplified by fixing the values of $p$ and $s$ to their pre-determined and estimated values. We also explore the sensitivity of the $\fnl$ posterior for the nonlinear three maps method to various values of $p$, ranging between $0.55$ and $1.6$, and $s$ from $0.75$ to $1.25$. Overall, as illustrated in Figure \ref{fig:fnl_magbias} and summarized in Table \ref{tab:dr9ps}, we find that the $\fnl$ posterior to be quite insensitive to the value of $s$, and both the best fitting estimate of $\fnl$ and the error increase as a function of $p$; however the overall impact on the signal to noise and $P(\fnl > 0)$ is shown to be insignificant.}
...
\caption{\mr{Best fitting estimates and $68\%$ ($95\%$) error on $\fnl$ from the DESI DR9 LRG sample with the non-linear three maps cleaning approach. The fiducial values are marked by stars. The constraints are stable against different values of $s$, but show noticeable sensitivity to $p$. As the value of $p$ increases the local PNG term decreases and thus the constraints get relaxed.}}\label{fig:fnl_magbias}
\end{figure}
