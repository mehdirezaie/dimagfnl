...
\begin{figure}
    ...
    \caption{Calibrated constrains from the DR9 LRG sample. \textit{Top}: probability distribution for $\fnl$ marginalized over the shotnoise and bias. \textit{Bottom}: probability contours for the bias from the DR9 LRG sample before and after applying nonlinear cleaning methods. The dark and light 2D contours represent $68\%$ and $95\%$ confidence intervals, respectively.}\label{fig:mcmc_dr9}
\end{figure}
...
\caption{True $\fnl$ vs measured $\fnl$ from the $\fnl=0$ and $76.9$ mocks with (dashed) and without systematics (solid). The solid lines represent the best fit from fitting the mean spectra and the scatters show the best fit estimates from fitting individual mocks.}\label{fig:fnlbias}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{figures/mcmc_cont.pdf}
\caption{Shifted probability distributions of $\fnl$ from the $\fnl=0$ mocks.}\label{fig:contmcmc}
\end{figure}